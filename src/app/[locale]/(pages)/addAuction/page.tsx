...
import { ImageIcon, Package, MapPin, MessageSquare, Gavel, ChevronDownIcon } from "lucide-react";
...
const AddAuctionPage: React.FC = () => {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (Array.isArray(values.contactMethods)) {
      values.contactMethods.forEach((m) => data.append("communication_methods[]", String(m)));
    }

    // auction end date: prefer form value, fallback to local date state
    const auctionEnd = values.auctionDuration || (date ? date.toISOString() : "");
    if (auctionEnd) {
      ...
      <div className="max-w-8xl mx-auto py-6 px-4 border-2 border-primary rounded-3xl bg-white/80">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            >
              <div>
                <h3 className="text-sm font-medium text-gray-700 mb-3 text-center">{t("Images.MainImageTitle")}</h3>
                <CustomUploadImageField type="single" typeSingle="square" control={form.control} />
              </div>

              <Separator />

              <div>
                <h3 className="text-sm font-medium text-gray-700 mb-3">{t("Images.OtherImagesTitle")}</h3>
                <CustomUploadImageField type="photoGroup" control={form.control} />
              </div>
            </CustomCard>

            {/* Product Details Section */}
            <CustomCard bgColor="Gradient_Card_Teal" title={t("Details.Title")} description={t("Details.Description")} icon={<Package className="w-5 h-5 text-primary" />}>
            ...
            <CustomCard bgColor="Gradient_Card_Teal" title={t("Location&Contact.Title")} description={t("Location&Contact.Description")} icon={<MapPin className="w-5 h-5 text-primary" />}>
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1">
                    ...
                    <ToggleContact control={form.control} name="contactMethods" label={t("Location&Contact.ContactLabel")} />
                  </div>
                </div>
              </div>
            </CustomCard>
            ...
            <div className="Gradient_Card_Teal cursor-auto shadow-none p-8 rounded-2xl flex flex-col justify-center items-center gap-4">
              <p className="max-w-[250px] mx-auto text-center text-lg font-medium">{t("ProductState.Title")}</p>
              <span className="text-lg text-primary font-bold">{t("ProductState.Price")}</span>
            </div>
            ...
            <Card className="lg:col-span-2 Gradient_Card_Teal cursor-auto shadow-none">
              <CardContent className="pt-6">
                <div className="text-center space-y-4">
                  ...
                  <Button size="lg" type="submit" className="w-full max-w-md h-12 text-lg font-medium" disabled={isLoading}>
                    {isLoading ? <LoaderSpin type="Btn" size="sm" title={t("Submit.Loading")} /> : t("Submit.SubmitButton")}
                  </Button>

                  <p className="text-sm text-gray-500 max-w-md mx-auto">{t("Submit.Description")}</p>
                </div>
              </CardContent>
            </Card>
          </form>
